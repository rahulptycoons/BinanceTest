...
# Start up the server to expose the metrics.
    ...
    # To Print Details
    bt.get_top_symbols('BTC','volume',True)
    bt.get_top_symbols('USDT', 'count', True)
    bt.get_notional_value('BTC', 'volume', True)

    uri = "/v3/ticker/24hr"

    r = requests.get(bt.API_URL + uri)
    df = pd.DataFrame(r.json())


    symbol=ps.sqldf("select  (substr(symbol,1,3)) as sym from df ")
    for col in symbol.values:
        bt.get_spread_delta(col[0], 'count', True)
